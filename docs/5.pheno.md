...
*    --ld-window-r2   The variant pairs with r2 values < 0.2 are filtered out by default. This option is used to adjust this threshold.
...
We will prune pairwise SNPs with R2 >  0.8 (using windows of 100kb and step size of 1 SNP)
...
How does the LD Decay Plot look like?
```
...
The `--pca` function in Plink2 extracts the top principal components from the variance-standardized relationship matrix (i.e. genomic relationship matrix). LD pruning is adviced before PCA to reduce the risk of getting PCs based on just a few genomic regions. This behaviour tends to prevent deflation of statistical testing results later.
