...
```
cat "$gwas"/${target}.to_be_excluded.ids | grep -Fwf - AxiomGT1v2.filtered.LD_prune.fam > "$gwas"/${target}.to_be_excluded.samples
grep ^Oldies AxiomGT1v2.filtered.LD_prune.fam >> "$gwas"/${target}.to_be_excluded.samples
...
plink2 --bfile AxiomGT1v2.filtered.LD_prune --chr-set 38 no-xy --allow-extra-chr \
      ...
      --make-bed --output-chr 'chrM' --out "$gwas"/AxiomGT1v2.filtered.LD_prune.${target}
```
There are 14517 variants and 2487 samples that passed filters and QC.


## 5.5. Exclusion of 1st degree relatives
Again, we will use the genotyping data to assess sample-distances and thus identify sample duplications. We will use 0.177 (the geometric mean of 0.25 and 0.125) as cutoff for the KING kinship coeffiecients to identify 1st degree relatives.

```
# First, we need to find the possible related dogs 
plink2 --bfile "$gwas"/AxiomGT1v2.filtered.LD_prune.${target} --chr-set 38 no-xy --allow-extra-chr \
       --king-cutoff 0.177 \
       --out "$gwas"/AxiomGT1v2.filtered.LD_prune.${target}.1st_degree_relatives

# Now, we can remove those relatives to avoid inflation of false associations in our GWAS
plink2 --bfile "$gwas"/AxiomGT1v2.filtered.LD_prune.${target} --chr-set 38 no-xy --allow-extra-chr \
       --remove "$gwas"/AxiomGT1v2.filtered.LD_prune.${target}.1st_degree_relatives.king.cutoff.out.id \
       --make-bed --output-chr 'chrM' --out "$gwas"/AxiomGT1v2.noRelatives.filtered.LD_prune.${target}
```
There are 14517 variants and 1982 samples (973 females, 1009 males) that passed filters and QC.


## 5.6. generate the phenotype file
```
...
cat "$gwas"/${target}.pheno | cut -d" " -f3 | sort | uniq -c ## we have 318 cases and 1664 controls
```

## 5.7. Assess distribution of cases in PCA
```
