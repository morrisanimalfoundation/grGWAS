...
# Some values may have commas so replace with semi-columns to make later bash processing easier  
wget https://raw.githubusercontent.com/morrisanimalfoundation/grGWAS/refs/heads/main/auxScripts/safeCSV.py
python safeCSV.py "phenotypes/conditions_${tissue}.csv" >  "phenotypes/conditions_${tissue}.safe.csv"

# Identify the 2nd row of the last year of study for each dog
cat phenotypes/conditions_${tissue}.safe.csv | awk 'BEGIN{FS=","}\
...
## 5.1.4. Which dogs had enough follow up?

Each dog has a record once enrolled in the study (year_in_study=0), then follow up records are added annually for 10 years. However, some dogs may drop out or miss some reports. This code calculates the number of records for each dog and the year of study for its last record.
```
# Calculate the number of records for each dog and the year of study for its last record
cat phenotypes/conditions_${tissue}.safe.csv  | sed 's/\".*\"//' | awk 'BEGIN{FS=OFS=","}\
    NR==1{print "subject_id,no_of_years,last_record_year,record_date";next}{if ($5==0 && $6!="") {\
    if (!($1 in max_year) || $3 > max_year[$1]) {
        max_year[$1] = $3;
        num_year[$1] += 1;
        split($4,a,";");
        if(a[1]) max_date[$1] = a[1];else max_date[$1] = "NA";
    }}}END{for (id in max_year) {print id,num_year[id],max_year[id],max_date[id];}}' > "$gwas"/conditions_${tissue}_studyYears.csv

# Merge with dog_profile data
awk 'BEGIN{FS=OFS=","}FNR==NR{a[$1]=$3 FS $6 FS $2;next}{if(a[$1])print $0,a[$1];else print $0,",,,"}' phenotypes/dog_profile.csv "$gwas"/conditions_${tissue}_studyYears.csv  > "$gwas"/conditions_${tissue}_studyYears_meta1.csv

# Calculate the age of the dog at the last reported record
wget https://raw.githubusercontent.com/morrisanimalfoundation/grGWAS/refs/heads/main/auxScripts/calc_age.py
python calc_age.py "$gwas"/conditions_${tissue}_studyYears_meta1.csv > "$gwas"/conditions_${tissue}_studyYears_meta2.csv
```


## 5.2. Sample selection

The GRLS dataset is a big cohort. Therefore, the case and control cohorts should be selected in a balanced design should to avoid several [statistical issues](https://morrisanimalfoundation.github.io/grGWAS/ext_docs/Imbalanced_GWAS_design/).
...
```
# remove controls if not examined enough or not matching the expected age range
awk 'BEGIN{FS=","}NR==FNR{a[$1]=1;next}{if ($1 && ! a[$1] && $2<5)print $1}'  "$gwas"/${target}_cases.ids "$gwas"/conditions_${tissue}_studyYears.csv > "$gwas"/${target}.to_be_excluded.ids

# remove controls with likely confounding co-existing conditions   
