...
It is meaningless to perform a genomic association study with a phenotype unless it is heritable. Preceding studies are needed to [determine heritability estimates specially for complex traits](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5635617/); however, this is out of scope for this tutorial. By heritability assessment, this means estimating the proportion of variance in our phenotype explained by the genotyped SNPs. Later, it should be calculated how much of this heritability was captured by the selected markers.   

[GCTA software](https://yanglab.westlake.edu.cn/software/gcta/#Overview) will be used to perform the GREML analysis for heritability assessment 
...
*  Data needs to be filtered to remove rare SNPs (e.g. --maf 0.01), and cryptic relatedness (e.g. --grm-cutoff 0.025). That dataset already created does not need any further filtration
...
## 6.3. Covariates
The data can be adjusted for quantitative and discrete covariates. Previous PCA analysis showed that three principle components explain most of the variance and none of them segregates with the phenotype of interest. Therefore, the GREML analysis for these 3 eigenvectors will be adjusted. Also, adjusments must be made for possible gender effect.

This already have this information has aready been obtained, but it needs to be reformated to match the GCTA expectations:
...
Accepted options:
--reml
--grm gcta/AxiomGT1v2.filtered.atopy.noRelatives.LD_prune.gcta_grm
...
Non-missing phenotypes of 1989 individuals are included from [gwas_output/atopy.pheno].
Reading quantitative covariate(s) from [gwas_output/3PCs.txt].
3 quantitative covariate(s) of 1989 individuals are included from [gwas_output/3PCs.txt].
Reading discrete covariate(s) from [gender.txt].
1 discrete covariate(s) of 1989 individuals are included from [gender.txt].
...
BLUP solutions of the genetic effects for 1989 individuals have been saved in the file [gwas_output/atopy_greml.indi.blp].
```
