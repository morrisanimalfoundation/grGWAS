...
## 7.1.1.  a logistic regression model
...
*  The model has a fixed-covariate matrix to include covariates loaded by `--covar`.
    ...
    *  You can add the 'sex' modifier to include ".fam/.psam" sex as a covariate. If you don't want this, add the 'no-x-sex' modifier instead 
*  Dataset precautions:
...
*  We typically use covariates to adjust for their effect but we do not need their effect in the output file. The 'hide-covar' modifier can greatly reduce file sizes.
...
Options in effect:
  --allow-extra-chr
  --bfile gwas_output/AxiomGT1v2.filtered.atopy.noRelatives.LD_prune
  --chr 1-38, X
  --chr-set 38 no-xy
  --ci 0.95
  --covar gwas_output/AxiomGT1v2.filtered.atopy.noRelatives.LD_prune.pca.eigenvec
  --covar-name PC1,PC2,PC3
  --glm sex hide-covar
  --out gwas_output/AxiomGT1v2.filtered.atopy.noRelatives.LD_prune.glm
  --output-chr 26
  --pheno gwas_output/atopy.pheno

....
1989 samples (980 females, 1009 males; 1989 founders) loaded from
gwas_output/AxiomGT1v2.filtered.atopy.noRelatives.LD_prune.fam.
180605 out of 180689 variants loaded from
gwas_output/AxiomGT1v2.filtered.atopy.noRelatives.LD_prune.bim.
1 binary phenotype loaded (317 cases, 1672 controls).
3 covariates loaded from gwas_output/AxiomGT1v2.filtered.atopy.noRelatives.LD_prune.pca.eigenvec.
Calculating allele frequencies... done.
--glm logistic regression on phenotype 'PHENO1': done.
Results written to gwas_output/AxiomGT1v2.filtered.atopy.noRelatives.LD_prune.glm.PHENO1.glm.logistic .
```
...
```
gcta64 --mlma-loco --bfile gcta/AxiomGT1v2.filtered.${target}.noRelatives.LD_prune.gcta --autosome-num 38 --autosome \
       --grm gcta/AxiomGT1v2.filtered.${target}.noRelatives.LD_prune.gcta_grm \
       --pheno $gwas/${target}.pheno \
