...
tail -n+2 gwas/AxiomGT1v2.noRelatives.filtered.LD_prune.atopy.glm.PHENO1.glm.logistic | awk '{print $13*$13}' | sort | awk '{count[NR] = $1;}\
END{
if(NR % 2){gif = count[(NR + 1) / 2];} else {gif = (count[(NR / 2)] + count[(NR / 2) + 1]) / 2.0;}
print "Genomic inflation factor = " gif/0.456;}'
```

Genomic inflation factor = 1.08413 which is great!
...
Now, we can open python in interactive mode by using the command `python`. Once we are dome, we can exit the interactive mode by using the command `exit`
...
gwas_results = pd.read_table("gwas/atopy_asc.loco.mlma", delim_whitespace=True)
...
Genomic control λ: 0.9888753019675566 suggesting minimal deflation in the test statistics. This could happen because of overcorrection for Population Stratification and/or other covariates 
